...
from pysinewave import beatwave_generator

class BeatWave:
    '''Generates and plays a continuous sinewave, with smooth transitions in frequency (pitch)
        and amplitude (volume).'''

    def __init__(self, pitch=0, beat_frequency=2, pitch_per_second=12, decibels=0, decibels_per_second=1, channels=1, channel_side="lr",
                samplerate=utilities.DEFAULT_SAMPLE_RATE):

        self.beatwave_generator = beatwave_generator.BeatWaveGenerator(
                                    pitch=pitch, beat_frequency=beat_frequency, pitch_per_second=pitch_per_second,
    ...
    def _callback(self, outdata, frames, time, status):
        ...
        data = self.beatwave_generator.next_data(frames)
